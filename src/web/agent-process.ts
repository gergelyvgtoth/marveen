...
// Local-only "does this exact tmux session exist" check, used by the dashboard
// agent-console and the health route (both inspect local sessions only).
...
//
// host (optional): when set, the capture runs on a remote laptop over ssh
// (routed through captureTmux). Null = local.
//
...
export function capturePane(session: string, host: string | null = null, scrollbackLines?: number): string | null {
  try {
    const args = ['capture-pane', '-t', session, '-p']
    if (scrollbackLines && scrollbackLines > 0) args.push('-S', `-${scrollbackLines}`)
    return captureTmux(host, args)
  } catch {
    return null
  }
}
